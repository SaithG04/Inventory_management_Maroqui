package ucv.app_inventory.login.aspect;
...
import ucv.app_inventory.login.audit.AuditService;
...
public class AuditAspect {
    ...
    public AuditAspect(AuditService auditService, UserService userAPIClient) {
    ...
    public Long getAuthenticatedUser() {
        ...
        return userAPIClient.findByEmail(email).orElseThrow().getIdUser();
    }
     ...
     */
    @AfterReturning(value = "execution(* ucv.app_inventory.login.application.*.create*(..))", returning = "result")
     ...
     */
    @AfterReturning(value = "execution(* ucv.app_inventory.login.application.*.update*(..))", returning = "result")
     ...
     */
    @Before("execution(* ucv.app_inventory.login.application.*.delete*(..))")
